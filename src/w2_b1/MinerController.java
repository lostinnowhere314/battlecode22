...
public class MinerController extends Robot {
	...
	// Vars for finding best location
	...
	public void run(RobotController rc) throws GameActionException {
		...
		adjResources = new MapLocation[MAX_MINEABLE];
		isGold = new boolean[MAX_MINEABLE];
		adjIndex = MAX_MINEABLE;
		enemyLocation = null;
		unimportantEnemy = true;
				...
				if (messagePos < 0) {
					...
					for (int i=64; --i>=56;) {
						...
						if (rc.readSharedArray(i)==0) { //prioritize empty slots
							...
							val = ((1+rc.getRoundNum() - (rc.readSharedArray(i) >> 12)) % 15);
						}
	...
	private MapLocation findBestDestination(RobotController rc) throws GameActionException {
		MapLocation me = rc.getLocation();
		...
		String indicatorString =  Integer.toString(leadLocs.length) + ";" + Integer.toString(goldLocs.length)+ ";" + Integer.toString(robots.length) + ";";
		rc.setIndicatorString(indicatorString);
