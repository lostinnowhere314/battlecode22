...
public class SoldierPathfinding {
	...
	public static Direction getBestDirection(RobotController rc, MapLocation dest, Direction lastMoveDirection) throws GameActionException {
		...
		if (lastMoveDirection == null || lastMoveDirection == Direction.CENTER) {
			lastMoveDirection = me.directionTo(dest);
		}
		
		switch(lastMoveDirection) {
			case NORTH:
				currentDistance = dist_0_4 + 3*Math.sqrt(loc_0_4.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_0_4;
				}
				
				currentDistance = dist_n1_4 + 3*Math.sqrt(loc_n1_4.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_n1_4;
				}
				
				currentDistance = dist_1_4 + 3*Math.sqrt(loc_1_4.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_1_4;
				}
				
				currentDistance = dist_n2_4 + 3*Math.sqrt(loc_n2_4.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_n2_4;
				}
				
				currentDistance = dist_2_4 + 3*Math.sqrt(loc_2_4.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_2_4;
				}
				
				currentDistance = dist_n2_3 + 3*Math.sqrt(loc_n2_3.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_n2_3;
				}
				
				currentDistance = dist_2_3 + 3*Math.sqrt(loc_2_3.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_2_3;
				}
				
				currentDistance = dist_n3_3 + 3*Math.sqrt(loc_n3_3.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_n3_3;
				}
				
				currentDistance = dist_3_3 + 3*Math.sqrt(loc_3_3.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_3_3;
				}
				
				currentDistance = dist_n3_2 + 3*Math.sqrt(loc_n3_2.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_n3_2;
				}
				
				currentDistance = dist_3_2 + 3*Math.sqrt(loc_3_2.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_3_2;
				}
				
				currentDistance = dist_n4_2 + 3*Math.sqrt(loc_n4_2.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_n4_2;
				}
				
				currentDistance = dist_4_2 + 3*Math.sqrt(loc_4_2.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_4_2;
				}
				
				currentDistance = dist_n4_1 + 3*Math.sqrt(loc_n4_1.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_n4_1;
				}
				
				currentDistance = dist_4_1 + 3*Math.sqrt(loc_4_1.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_4_1;
				}
				
				currentDistance = dist_n4_0 + 3*Math.sqrt(loc_n4_0.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_n4_0;
				}
				
				currentDistance = dist_4_0 + 3*Math.sqrt(loc_4_0.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_4_0;
				}
				
				currentDistance = dist_n4_n1 + 3*Math.sqrt(loc_n4_n1.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_n4_n1;
				}
				
				currentDistance = dist_4_n1 + 3*Math.sqrt(loc_4_n1.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_4_n1;
				}
				
				currentDistance = dist_n3_n2 + 3*Math.sqrt(loc_n3_n2.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_n3_n2;
				}
				
				currentDistance = dist_3_n2 + 3*Math.sqrt(loc_3_n2.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_3_n2;
				}
				
				currentDistance = dist_n4_n2 + 3*Math.sqrt(loc_n4_n2.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_n4_n2;
				}
				
				currentDistance = dist_4_n2 + 3*Math.sqrt(loc_4_n2.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_4_n2;
				}
				
				currentDistance = dist_n2_n3 + 3*Math.sqrt(loc_n2_n3.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_n2_n3;
				}
				
				currentDistance = dist_2_n3 + 3*Math.sqrt(loc_2_n3.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_2_n3;
				}
				
				currentDistance = dist_n3_n3 + 3*Math.sqrt(loc_n3_n3.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_n3_n3;
				}
				
				currentDistance = dist_3_n3 + 3*Math.sqrt(loc_3_n3.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_3_n3;
				}
				
				currentDistance = dist_0_n4 + 3*Math.sqrt(loc_0_n4.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_0_n4;
				}
				
				currentDistance = dist_n1_n4 + 3*Math.sqrt(loc_n1_n4.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_n1_n4;
				}
				
				currentDistance = dist_1_n4 + 3*Math.sqrt(loc_1_n4.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_1_n4;
				}
				
				currentDistance = dist_n2_n4 + 3*Math.sqrt(loc_n2_n4.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_n2_n4;
				}
				
				currentDistance = dist_2_n4 + 3*Math.sqrt(loc_2_n4.distanceSquaredTo(dest));
				if (bestDistance > currentDistance) {
					bestDistance = currentDistance;
					bestDirection = direc_2_n4;
				}
